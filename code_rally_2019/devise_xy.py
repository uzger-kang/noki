...
X_data = np.loadtxt('train.csv', skiprows=1, delimiter=',',
                    usecols=(0,))
Y_data = np.loadtxt('train.csv', skiprows=1, delimiter=',',
                    usecols=(1,))
...
# 卷积层,64个卷积核
model.add(Convolution2D(nb_filter=64, nb_row=1, nb_col=2,
                        border_mode='same', input_shape=(1, n_need, 11*12)))
...
model.fit(BS_data, Y_data, nb_epoch=90, batch_size=256,
...
# 测试（针对训练集损失）
loss, accuracy = model.evaluate(BS_data, Y_data)
